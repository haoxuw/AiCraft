...
# or: ./build/modcraft --skip-menu --debug-scenario item_views --debug-item base:sword
```
...
# or: ./build/modcraft --skip-menu --debug-scenario character_views --debug-character base:pig
```
...
> the `modcraft` target won't relink, so the CMake `POST_BUILD copy_directory
> src/ModCraft/artifacts → build/artifacts` step doesn't fire and the game
> reads the **stale cached model**. Two fixes:
>
> ```bash
> # Fast: sync just the edited file
> cp src/ModCraft/artifacts/models/base/pig.py build/artifacts/models/base/pig.py
>
> # Or: force a relink so POST_BUILD re-copies everything
> touch src/ModCraft/main.cpp && cmake --build build -j$(($(nproc)/2))
> ```
>
...
```bash
# All base models
python3 src/ModCraft/tools/check_zfight.py

# Just the characters, stricter tolerance (exact coplanar only)
python3 src/ModCraft/tools/check_zfight.py --tolerance 0.003 \
    src/ModCraft/artifacts/models/base/{cat,pig,dog,chicken,skeleton,knight,mage,villager,player,giant,crewmate}.py
```

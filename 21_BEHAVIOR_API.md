...
Every creature in AiCraft has a behavior defined as a Python file. The behavior's `decide()` function is called 4 times per second and returns an action (what the creature should do next).
...
```python
from aicraft_engine import Idle, Wander, MoveTo, Follow, Flee
```
...
```python
from aicraft_engine import Idle, Wander, Flee

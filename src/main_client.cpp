 ...
 *
 * Usage: ./modcraft-client [--host HOST] [--port PORT]
...
static void crashHandler(int sig) {
	...
	FILE* f = fopen("/tmp/modcraft_crash.log", "w");
	if (f) {
		...
		fclose(f);
		fprintf(stderr, "[CRASH] Backtrace written to /tmp/modcraft_crash.log\n");
	}
...
int main(int argc, char** argv) {
	...
	for (int i = 1; i < argc; i++) {
		if (strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "-h") == 0) {
			printf("ModCraft — network client\n\n"
			       "Usage: %s [options]\n"

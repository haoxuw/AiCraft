...
void App::startMatchFromLab() {
	// Build a one-off template pre-populated with the lab's cell, plates,
	// and parts so the regular match setup path works unchanged.
...
void App::drawMonsterSelect(float dt) {
	// Keep the screensaver sim running behind this screen too. If the user
	// reached MONSTER_SELECT directly (e.g. --select-screenshot) the sim
	// may not be initialized yet.
	if (!sim_ || world_.monsters.empty()) initMenuSim();
	stepMenuSim(dt);
	drawFood();
	drawMonsters();
	// Stronger dim here — the selection tiles need high contrast.
	text_->drawRect(-1.0f, -1.0f, 2.0f, 2.0f, glm::vec4(0.0f, 0.0f, 0.0f, 0.6f));
...
void App::drawEndScreen(float dt) {
	...
	float aspect = window_.aspectRatio();
	// Dim overlay + centered panel, same look as main menu.
	text_->drawRect(-1.0f, -1.0f, 2.0f, 2.0f, glm::vec4(0.0f, 0.0f, 0.0f, 0.55f));
	float px = -0.40f, py = -0.40f, pw = 0.80f, ph = 0.85f;
	text_->drawRect(px, py, pw, ph, glm::vec4(0.04f, 0.05f, 0.06f, 0.88f));
	auto border = [&](float x, float y, float w, float h, float t, glm::vec4 c) {
		text_->drawRect(x, y, w, t, c);
		text_->drawRect(x, y + h - t, w, t, c);
		text_->drawRect(x, y, t, h, c);
		text_->drawRect(x + w - t, y, t, h, c);
	};
	border(px, py, pw, ph, 0.004f, glm::vec4(0.92f, 0.90f, 0.78f, 0.95f));
	border(px + 0.012f, py + 0.012f, pw - 0.024f, ph - 0.024f, 0.002f,
		glm::vec4(0.70f, 0.68f, 0.58f, 0.8f));

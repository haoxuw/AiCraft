 ...
 *   1. Spawn agentica-server on a free port
...
public:
	struct Config {
		...
		std::string execDir;     // directory containing agentica-* binaries
	};
	...
	int launchServer(const Config& cfg) {
		...
		std::string serverBin = cfg.execDir + "/agentica-server";
		...
		char readyPath[64];
		snprintf(readyPath, sizeof(readyPath), "/tmp/agentica_ready_%d", m_port);
	...
	static int findFreePort() {
		for (int p = 7800; p < 7900; p++) {
			...
			if (free) {
				...
				char stale[64];
				snprintf(stale, sizeof(stale), "/tmp/agentica_ready_%d", p);
				std::remove(stale);
				return p;
			}
		}

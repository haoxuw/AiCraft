#!/bin/bash
# capture_samples.sh — drive solarium-ui-vk through representative game
# states and capture one PNG per state into $OUT_DIR.
#
# Uses only file-based debug triggers (no xdotool / keyboard injection):
#   /tmp/solarium_screenshot_request         — take a screenshot
#   /tmp/solarium_vk_camera_request          — cycle FPS→TPS→RPG→RTS
#   /tmp/solarium_vk_inventory_request       — toggle inventory panel
#   /tmp/solarium_vk_pause_request           — toggle pause
#   /tmp/solarium_vk_admin_request           — toggle admin
#   /tmp/solarium_vk_fly_request             — toggle fly (requires admin)
#   /tmp/solarium_vk_ascend_request          — +5 y (requires fly)
#
...
set -u
BUILD_DIR="${1:-build}"
OUT_DIR="${2:-/tmp/solarium_samples}"
BIN="$BUILD_DIR/solarium-ui-vk"
...
kill_game() {
	for p in $(pgrep -x solarium-ui-vk 2>/dev/null); do kill -9 "$p" 2>/dev/null || true; done
	for p in $(pgrep -x solarium-server 2>/dev/null); do kill -9 "$p" 2>/dev/null || true; done
...
# The VK screenshot writer names files sequentially: solarium_vk_screenshot_N.ppm.
# We bump $SHOT_N ourselves to track which file corresponds to which label.
SHOT_N=0
shoot() {
	local label="$1"
	rm -f /tmp/solarium_screenshot_request
	touch /tmp/solarium_screenshot_request
	...
	local src="/tmp/solarium_vk_screenshot_${SHOT_N}.ppm"
...
rm -f /tmp/solarium_vk_screenshot_*.ppm
kill_game
...
# SOLARIUM_NO_FOCUS_PAUSE=1 keeps gameplay ticking when the window isn't the
...
( cd "$BUILD_DIR" && SOLARIUM_NO_FOCUS_PAUSE=1 ./solarium-ui-vk ) >/tmp/solarium_samples_phase1.log 2>&1 &
PHASE1_PID=$!
sleep 3.0
shoot "01_main_menu"
...
( cd "$BUILD_DIR" && SOLARIUM_NO_FOCUS_PAUSE=1 ./solarium-ui-vk --skip-menu ) >/tmp/solarium_samples_phase2.log 2>&1 &
PHASE2_PID=$!
sleep 7.0   # world gen + chunk streaming + mesh upload

shoot "02_gameplay_fps"

trigger /tmp/solarium_vk_camera_request 0.6
...
trigger /tmp/solarium_vk_inventory_request 0.4
shoot "07_inventory_open"
trigger /tmp/solarium_vk_inventory_request 0.4  # close

# Pause overlay.
trigger /tmp/solarium_vk_pause_request 0.4
shoot "08_paused"
trigger /tmp/solarium_vk_pause_request 0.4  # resume

# Admin fly — gives a top-down peek at the village.
trigger /tmp/solarium_vk_admin_request 0.3
trigger /tmp/solarium_vk_fly_request   0.3
for _ in 1 2 3 4 5 6; do trigger /tmp/solarium_vk_ascend_request 0.15; done
trigger /tmp/solarium_vk_camera_request 0.6   # RPG gives a clean overhead
trigger /tmp/solarium_vk_camera_request 0.6   # RTS — bird's eye

...
// Civ6 Civilopedia / Pokédex chrome — sidebar pinned to the left edge holds
// the entry list, the right area is mostly transparent so a live 3D preview
// shows through, and the bottom flushes against both edges as a brass-
// bordered "horizon" panel. Shared by the Handbook and the Character Select
// page (both want the same browse-then-detail interaction model).
//
// Self-contained — does NOT compose on top of baseCss(). The two pages have
// fundamentally different body layouts (centred column vs sidebar+content)
// so each ships its own typography reset.
inline std::string dexCss() {
	return
		// Body reset — sidebar pages need block flow, not the centred
		// flex column baseCss() uses.
		...
		".sb-head{padding:22px 22px 12px;"
		"border-bottom:1px solid rgba(184,136,56,0.4)}"
		".sb-head h1{font-size:24px;letter-spacing:4px;margin:0;"
		"color:var(--accent-hi);font-weight:400;"
		"text-shadow:0 2px 8px rgba(0,0,0,0.8)}"
		".sb-head .sub{font-size:11px;letter-spacing:2px;"
		"color:var(--accent-mid);text-transform:uppercase;margin-top:4px}"
		".sb-search{margin:14px 22px 8px;padding:8px 12px;width:auto;"
		"background:rgba(20,13,8,0.97);color:var(--ink);"
		"border:1px solid var(--accent-mid);font-family:inherit;"
		"font-size:13px;letter-spacing:1px;box-sizing:border-box}"
		...
		"border-top:0;border-right:0;border-bottom:0;text-align:left;"
		...
		"opacity:0.7}"
		".sb-foot{padding:14px 22px;"
		"border-top:1px solid rgba(184,136,56,0.4);"
		...
		".sb-foot .primary{color:var(--accent-hi);font-size:16px;"
		"padding:14px 0;box-shadow:0 0 0 1px var(--accent-hi) inset}"

		// ── Detail card (right pane) ─────────────────────────────────────
		".detail{margin-left:300px;min-height:100vh;position:relative}"
		// 48vh leaves the upper ~52vh for the live 3D preview model
		// while the panel still fits description + first ~7 stat rows
		// + the EDIT SOURCE button (rest is in the scrollable attrs).
		...
		"letter-spacing:3px;color:var(--accent-hi);"
		"text-transform:uppercase;font-family:monospace;margin-right:12px;"
		"padding:3px 9px;background:rgba(0,0,0,0.6);"
		"border:1px solid rgba(184,136,56,0.5)}"
		".detail-card h2{font-size:56px;letter-spacing:6px;"
		"color:var(--accent-hi);margin:8px 0 0;font-weight:400;"
		"text-shadow:0 4px 18px rgba(0,0,0,0.95),"
		"0 1px 0 rgba(0,0,0,0.9)}"
		// Flushed-to-edge panel. Brass border lives only on the top edge
		// (the floor's "horizon line") — sides are bounded by the
		// sidebar and screen edge, so the panel reads as architecturally
		// connected rather than a floating tile.
		...
		"padding:22px 60px;display:flex;flex-direction:column;gap:12px;"
		...
		"font-size:12px;font-family:monospace;flex:1;overflow-y:auto;"
		"padding-right:8px}"
		...
		// ── Artistic scrollbar (track + thumb + arrow buttons) ──────────
		// Same recipe as baseCss() for visual continuity across pages.
		"::-webkit-scrollbar{width:14px;height:14px}"
		"::-webkit-scrollbar-track{"
		"background:linear-gradient(to right,"
		"rgba(184,136,56,0.55) 0,rgba(184,136,56,0.55) 1px,"

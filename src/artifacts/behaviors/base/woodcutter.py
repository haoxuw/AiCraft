...
from civcraft_engine import Move, Convert, Block, scan_blocks, scan_entities, get_block
...
class WoodcutterBehavior(Behavior):
    ...
    def _work(self, entity: SelfEntity, local_world: LocalWorld):
        ...
        if self._tree_target is None:
            ...
            prefer  = "leaves" if random.random() < 0.5 else "logs"
            fallback = "logs" if prefer == "leaves" else "leaves"
        ...
        label = "leaves" if tid == "leaves" else "log"

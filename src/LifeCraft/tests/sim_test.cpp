// LifeCraft — headless sim smoke test. Drives two players into a
// contact fight, asserts that bite + kill events fire, biomass
// transfers, and food pickups happen.
//
// Build target: lifecraft-sim-test. Run with no args. Exits 0 on pass.
...
void fail(const char* msg) {
	std::fprintf(stderr, "lifecraft-sim-test FAIL: %s\n", msg);
	std::exit(1);
}
} // namespace
...
int main() {
	...
	std::printf("lifecraft-sim-test PASS  "

...
class Wander(Action):
    ...
    Optional biasing + despawn:
      target_block    Search for a block/annotation of this type (str or list
                      of str). If found within `search_radius`, wander within
                      `radius` of that target instead of the entity's position.
                      Matches blocks AND annotations (flowers, moss, …) via
                      the unified w.all() API.
      search_radius   Distance cap for target_block lookup. Default 24.
      despawn_after   If set and no target is found for this many seconds, the
                      entity despawns (Convert hp → nothing). Used for loose
    ...
    def tick(self, dt):
        # Counter is reset by run() each time a target is actually found.
        # Nothing to do here — run() owns the target/no-target transition.
        pass

    def _find_target(self, w):
        if self.target_block is None:
            return None
        types = self.target_block if isinstance(self.target_block, (list, tuple)) \
                else [self.target_block]
        best = None
        for t in types:
            for h in w.all(t, max_dist=self.search_radius):
                if best is None or h.distance < best.distance:
                    best = h
        return best

    def run(self, e, w, ctx):
        dt = getattr(w, "dt", 0.0) or 0.0
        target = self._find_target(w)

        if target is None and self.target_block is not None:
            self._no_target_secs += dt
            # Despawn gate: convert our own hp away so the engine removes us.
            if self.despawn_after is not None and self._no_target_secs > self.despawn_after:
                hp = max(int(getattr(e, "hp", 0)), 1)
                return (Convert(from_item="hp", from_count=hp,
                                to_item="", to_count=0,
                                convert_from=Self()),
                        "Despawning (no target)")
        else:
            self._no_target_secs = 0.0

        # Pick a random point either around the target or around self.
        if target is not None:
            cx, cz = float(target.x) + 0.5, float(target.z) + 0.5
        else:
            cx, cz = e.x, e.z
...
class SeekPerch(Action):
    ...
    def __init__(self, block_types=("wood", "planks", "fence"),

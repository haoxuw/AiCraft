...
class BraveChickenBehavior(RulesBehavior):
    def __init__(self):
        super().__init__()
        self.rules = [
            (Threatened(range=6, types=["base:dog"]),         Flee(distance=12, speed_mul=2.0,
                                                                    message="EEK! Dog!")),
            (IsEveningOrNight() & FarFromHome(radius=3),      GoHome(message="Heading home to roost...")),
            (IsEveningOrNight(),                              Rest(message="Roosting zzz")),
            (NearType("base:player", range=3),                LayEgg(chance=0.15, cooldown=8.0,
                                                                      message="*happy cluck* Laid an egg!")),
            (NearType("base:player", range=20),               Follow(target_type="base:player",
                                                                      close_dist=3,
